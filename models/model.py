...
from models.modules.error_transformer import ErrorAwareTransformerBlock, SoftSplit, SoftComp
...
class GatedEncoder(nn.Module):
    def __init__(self):
        ...
        self.new_gating_layers = nn.ModuleList([
            nn.Conv2d(5, 64, 3, 2, 1, bias=False),
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            out = layer(out)
            if i< 8 and i % 2 == 0:
                gating = torch.sigmoid(self.new_gating_layers[cnt](shortcut))
                out = out * gating
                cnt += 1
        return out, dense
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        window_size = (5, 9)
        pool_size = (4, 4)
        self.transformers = ErrorAwareTransformerBlock(dim=hidden,
                                                n_head=num_heads,
    ...
    def forward(self, masked_frames, completed_flows, masks_in, masks_updated, num_local_frames, memory_frames, memory_masks, interpolation='bilinear', t_dilation=2):
        """
        Args:
        ...
        enc_feat, dense_feat = self.encoder(torch.cat([masked_frames.view(b * t, 3, ori_h, ori_w), masks_in.view(b * t, 1, ori_h, ori_w), masks_updated.view(b * t, 1, ori_h, ori_w)], dim=1))
        b, m_t, _, _, _ = memory_frames.shape
        
        
        enc_memory, dense_memory = self.encoder(torch.cat([memory_frames.view(b * m_t, 3, ori_h, ori_w), memory_masks.view(b * m_t, 1, ori_h, ori_w), torch.zeros_like(memory_masks.view(b * m_t, 1, ori_h, ori_w))], dim=1))
        
        _, c, h, w = enc_memory.size()
        fold_feat_size = (h, w)
        ...
        trans_memo = self.ss(enc_memory.view(-1, c, h, w), b, fold_feat_size)

        mask_pool_l = rearrange(mask_pool_l, 'b t c h w -> b t h w c').contiguous()

        trans_feat, trans_memo, upd_mask = self.transformers(trans_feat, trans_memo, l_t, fold_feat_size, mask_pool_l, t_dilation=t_dilation, raw_img=masked_frames)
        
        
        trans_feat = self.sc(trans_feat, t, fold_feat_size)
        trans_feat = trans_feat.view(b, t, -1, h, w)
        enc_feat = (enc_feat + trans_feat).view(-1, c, h, w)
        
        if trans_memo is not None:
            trans_memo = self.sc(trans_memo, m_t, fold_feat_size)
            trans_memo = trans_memo.view(b, m_t, -1, h, w)
            enc_memory = (enc_memory + trans_memo).view(-1, c, h, w)
        
    
        for i, layer in enumerate(self.decoder):
            if i == 4:
                enc_feat = torch.cat((enc_feat, dense_feat), 1)
                if trans_memo is not None:
                    enc_memory = torch.cat((enc_memory, dense_memory), 1)
            enc_feat = layer(enc_feat)
            if trans_memo is not None:
                enc_memory = layer(enc_memory)
        
        output = torch.tanh(enc_feat).view(b, t, 3, ori_h, ori_w)
        output_memory = torch.tanh(enc_memory).view(b, m_t, 3, ori_h, ori_w)
        return output, output_memory, upd_mask
